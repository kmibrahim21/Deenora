...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    try {
      const { id } = req.query;
      const response = await fetch(`${AWAJ_BASE_URL}/surveys/${id}/result`, { headers: getAwajHeaders() });
      if (!response.ok) {
        console.error(`Awaj API Error (Survey Result): ${response.status} ${response.statusText}`);
