...
-- These indices ensure that RLS filters (which always check institution_id) are lightning fast.

-- Students: Optimized for class listing and roll-based sorting
CREATE INDEX IF NOT EXISTS idx_students_tenant_class_roll 
ON public.students (institution_id, class_id, roll);

-- Students: Optimized for name search within an institution
CREATE INDEX IF NOT EXISTS idx_students_tenant_name_search 
ON public.students USING gin (institution_id, student_name gin_trgm_ops);

-- Attendance: Optimized for date range queries within an institution
CREATE INDEX IF NOT EXISTS idx_attendance_tenant_date 
ON public.attendance (institution_id, date, student_id);

-- Ledger: Optimized for financial reporting
CREATE INDEX IF NOT EXISTS idx_ledger_tenant_date_type 
ON public.ledger (institution_id, transaction_date DESC, type);
...
CREATE OR REPLACE FUNCTION get_tenant_student_summary(p_institution_id UUID, p_class_id UUID DEFAULT NULL)
RETURNS TABLE (
    total_count BIGINT,
    active_count BIGINT
) AS $$
