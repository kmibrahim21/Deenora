...
BEGIN
    ...
    att_metrics AS (
        ...
        WHERE s.institution_id = p_madrasah_id
        GROUP BY s.id
    ),
    ...
    latest_exams AS (
        ...
        FROM public.exams e
        WHERE e.institution_id = p_madrasah_id
...
CREATE OR REPLACE FUNCTION get_smart_result_insights(p_institution_id UUID)
RETURNS TABLE (
    student_id UUID,
    student_name TEXT,
    class_name TEXT,
...
-- Smart Attendance Insights Function
CREATE OR REPLACE FUNCTION get_smart_attendance_insights(p_institution_id UUID)
RETURNS TABLE (
    student_id UUID,
    student_name TEXT,
    class_name TEXT,
...
AS $$
DECLARE
    v_current_start DATE := CURRENT_DATE - INTERVAL '15 days';
    v_previous_start DATE := CURRENT_DATE - INTERVAL '30 days';
    v_previous_end DATE := CURRENT_DATE - INTERVAL '16 days';
BEGIN
    RETURN QUERY
    WITH current_metrics AS (
        SELECT 
            ...
            s.roll,
            COUNT(a.id) as total,
            COUNT(a.id) FILTER (WHERE a.status IN ('present', 'late')) as active
        FROM public.students s
        JOIN public.classes c ON s.class_id = c.id
        LEFT JOIN public.attendance a ON s.id = a.student_id AND a.date >= v_current_start
        WHERE s.institution_id = p_institution_id
        GROUP BY s.id, s.student_name, c.class_name, s.roll
    ),
    previous_metrics AS (
        SELECT 
            s.id as s_id,
            ...
            COUNT(a.id) FILTER (WHERE a.status IN ('present', 'late')) as active
        FROM public.students s
        LEFT JOIN public.attendance a ON s.id = a.student_id AND a.date BETWEEN v_previous_start AND v_previous_end
        WHERE s.institution_id = p_institution_id
        GROUP BY s.id
    )
    SELECT 
        ...
        cm.class_name,
        cm.roll,
        COALESCE(CASE WHEN cm.total > 0 THEN (cm.active::NUMERIC / cm.total::NUMERIC) * 100 ELSE 100 END, 100)::NUMERIC as current_pct,
        COALESCE(CASE WHEN pm.total > 0 THEN (pm.active::NUMERIC / pm.total::NUMERIC) * 100 ELSE 100 END, 100)::NUMERIC as previous_pct,
        (COALESCE(CASE WHEN cm.total > 0 THEN (cm.active::NUMERIC / cm.total::NUMERIC) * 100 ELSE 100 END, 100) - 
         COALESCE(CASE WHEN pm.total > 0 THEN (pm.active::NUMERIC / pm.total::NUMERIC) * 100 ELSE 100 END, 100))::NUMERIC as change,
        CASE 
            WHEN (cm.active::NUMERIC / NULLIF(cm.total, 0)) - (pm.active::NUMERIC / NULLIF(pm.total, 0)) > 0.1 THEN 'improving'
            WHEN (cm.active::NUMERIC / NULLIF(cm.total, 0)) - (pm.active::NUMERIC / NULLIF(pm.total, 0)) < -0.1 THEN 'declining'
            ELSE 'stable'
        END as status
    FROM current_metrics cm
    LEFT JOIN previous_metrics pm ON cm.s_id = pm.s_id
    ORDER BY change ASC;
END;
$$;
