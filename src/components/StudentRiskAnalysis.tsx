...
import { Language, Student, Institution } from 'types';
...
const localT = {
  analyzingRisks: { en: 'Analyzing Risks...', bn: 'ঝুঁকি বিশ্লেষণ করা হচ্ছে...' },
  showLess: { en: 'Show Less', bn: 'কম দেখান' },
  showAll: { en: 'Show All', bn: 'সব দেখান' },
};
...
const StudentRiskAnalysis: React.FC<RiskAnalysisProps> = ({ institutionId, lang, onStudentClick, madrasah }) => {
  ...
  if (loading) return (
    <div className={`${madrasah?.theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-6 rounded-[2.2rem] border shadow-bubble flex items-center justify-center gap-3`}>
      <Loader2 className="animate-spin text-[#2563EB]" size={20} />
      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{localT.analyzingRisks[lang]}</span>
    </div>
  );
      ...
      <div className="flex items-center justify-between px-3">
        ...
        { (highRisks.length > 3 || warningRisks.length > 0) && (
          <button 
            ...
            className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${madrasah?.theme === 'dark' ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-50 text-[#2563EB]'}`}
          >
            {expanded ? localT.showLess[lang] : localT.showAll[lang]}
          </button>
        )}
      </div>
      ...
      <div className="space-y-2.5">
        {displayedRisks.length > 0 ? displayedRisks.map((risk) => (
          <div 
            ...
            className={`${madrasah?.theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-4 rounded-[1.8rem] border shadow-bubble flex items-center justify-between active:scale-[0.98] transition-all group cursor-pointer`}
          >
            <div className="flex items-center gap-4 min-w-0 flex-1">
              <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${
                ...
                ? (madrasah?.theme === 'dark' ? 'bg-red-500/20 text-red-400' : 'bg-red-50 text-red-500')
                : (madrasah?.theme === 'dark' ? 'bg-orange-500/20 text-orange-400' : 'bg-orange-50 text-orange-500')
              }`}>
                <AlertTriangle size={22} />
              </div>
              <div className="min-w-0">
                <h3 className={`font-black text-[15px] font-noto truncate leading-tight ${madrasah?.theme === 'dark' ? 'text-slate-200' : 'text-[#1E3A8A]'}`}>{risk.student_name}</h3>
          ...
          <div className={`${madrasah?.theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-8 rounded-[2.2rem] border shadow-bubble text-center`}>
